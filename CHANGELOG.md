...
* **content_router:** graph-scoped narrowing of discovery-tool output (`Grep`/`Glob`/`LS`). `ContentRouter` now filters wide discovery results down to files within `graph_narrow_max_hops` import-hops of the agent's most recently read file, delegating the import-graph query to the existing `codebase-memory-mcp` integration (`headroom/graph_context.py`) instead of re-implementing per-language parsing. Always CCR-recoverable — dropped lines collapse behind a `<<ccr:HASH N_rows_offloaded>>` marker, never discarded, so the agent can retrieve the full listing on demand. Degrades to zero-cost pass-through whenever narrowing can't be done with confidence (no entry file yet, output already narrow, codebase-memory-mcp unavailable, or the query fails). Configurable via `ContentRouterConfig.enable_graph_narrow` / `graph_narrow_max_hops` / `graph_narrow_min_lines` ([#1925](https://github.com/chopratejas/headroom/issues/1925)).
